...
Credentials are in `.env` (SUPABASE_URL, SUPABASE_ANON_KEY). Query the inventory
over the REST API:
...
- Writes (insert/update) via the same REST API are fine — the anon key has full access to `pantry_items`.
